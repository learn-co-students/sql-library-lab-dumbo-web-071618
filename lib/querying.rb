...
def select_name_and_series_subgenres_of_authors
  "SELECT authors.name, subgenres.name FROM  series INNER JOIN subgenres ON series.subgenre_id = subgenres.id
...
def select_series_title_with_most_human_characters
  ...
  GROUP BY series.id
  ORDER BY COUNT('human') LIMIT 1
  ;"
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT() FROM characters INNER JOIN character_books ON characters.id = character_books.character_id
